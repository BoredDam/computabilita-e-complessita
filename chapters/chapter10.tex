...
\end{aligned}
	...
	Se $f$ è una funzione \textbf{unaria calcolabile}, $f = \phi_a$ per qualche $a \in \mathbb{N}$, con $a = \gamma(P)$. Diremo che $a$ è un indice per $f$. Siccome esistono infiniti programmi che calcolano la stessa funzione, la sequenza $\alpha_0, \alpha_1, \alpha_2, \dots$ è una sequenza \textbf{con ripetizioni} che contiene tutte le funzioni unarie calcolabili. Generalizzando anche le arietà, la sequenza $\alpha_0^{(n)}, \alpha_1^{(n)}, \alpha_2^{(n)}, \dots$ è una sequenza \textbf{con ripetizioni} che contiene tutte le funzioni $n$-arie calcolabili.
	
	\paragraph{Teorema}
	$C_n = \{\phi_a^{(n)}: a \in \mathbb{N}\}$, ossia l'insieme delle funzioni calcolabili, è enumerabile.
